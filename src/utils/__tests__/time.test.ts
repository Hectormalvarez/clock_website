...
import { formatTime, formatTimeForTitle, formatDuration, formatFinishTime } from '../time';

function fakeDate(year: number, month: number, day: number, h: number, m: number, s: number): Date {
...
describe('formatTimeForTitle', () => {
  ...
  it('formats evening time without seconds', () => {
    expect(formatTimeForTitle(fakeDate(2025, 7, 4, 17, 30, 45))).toBe('5:30 PM');
  });
});
