...
describe('startTimer', () => {
  ...
  it('transitions paused → running (resume)', () => {
    const result = startTimer(makeCore({ state: 'paused', remaining: 120 }), now);
    expect(result!.state).toBe('running');
    expect(result!.remaining).toBe(120);
  });

  it('does nothing if already running', () => {
    const result = startTimer(makeCore({ state: 'running', remaining: 100 }), now);
...
describe('resetTimer', () => {
  it('returns to idle with configured duration', () => {
    const core = makeCore({ configuredDuration: 600, remaining: 100, state: 'running' });
